...
void djcs_encrypt(djcs_public_key *pk, hcs_random *hr, mpz_t rop, mpz_t plain1, unsigned long s)
{
    mpz_t t1;
    mpz_init(t1);

    mpz_random_in_mult_group(t1, hr->rstate, pk->n[0]);
    mpz_powm(rop, pk->g, plain1, pk->n[s]);
    mpz_powm(t1, t1, pk->n[s-1], pk->n[s]);
    mpz_mul(rop, rop, t1);
    mpz_mod(rop, rop, pk->n[s]);
...
void djcs_ep_add(djcs_public_key *pk, mpz_t rop, mpz_t cipher1, mpz_t plain1,unsigned long s)
{
    mpz_t t1;
    mpz_init(t1);
...
void djcs_ee_add(djcs_public_key *pk, mpz_t rop, mpz_t cipher1, mpz_t cipher2,unsigned long s)
{
    mpz_mul(rop, cipher1, cipher2);
    mpz_mod(rop, rop, pk->n[s]);
}

void djcs_ep_mul(djcs_public_key *pk, mpz_t rop, mpz_t cipher1, mpz_t plain1,unsigned long s)
{
    mpz_powm(rop, cipher1, plain1, pk->n[s]);
}

void djcs_decrypt(djcs_private_key *vk, mpz_t rop, mpz_t cipher1,unsigned long s)
{
    mpz_powm(rop, cipher1, vk->d, vk->n[s]);
    dlog_s(vk, rop, rop);
    mpz_mul(rop, rop, vk->mu);
    mpz_mod(rop, rop, vk->n[s-1]);
}
